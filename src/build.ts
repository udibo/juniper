...
export interface BuildOptions {
  ...
  write?: boolean;
  /**
   * Options for configuring the React Router on the client side.
   * These options will be passed to the generated Client instance.
   */
  routerOptions?: {
    /**
...
export class Builder implements AsyncDisposable {
  ...
  /** Router options for configuring the React Router on the client side. */
  protected routerOptions?: BuildOptions["routerOptions"];
...
`));
  ...
  buildMainClientEntrypoint(): Promise<void> {
    return startActiveSpan("buildMainClientEntrypoint", async () => {
      ...
      const fmtWriter = fmt.stdin.getWriter();
      const encoder = new TextEncoder();
      const routerOptionsString = this.routerOptions
        ? `, ${generatedRouteObjectToString(this.routerOptions, 0)}`
        : "";
      fmtWriter.write(encoder.encode(`\
// This file is auto-generated by @udibo/juniper/build
// Do not edit this file directly.
...
export const client = new Client(${routesConfigString}${routerOptionsString});
`));
      fmtWriter.close();
      const { success, code } = await fmt.status;
      if (!success) {
