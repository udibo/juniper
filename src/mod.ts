 ...
 * `export const middleware` in a `.tsx` route: the builder imports those modules
 * eagerly so the router can install their middleware. A form it cannot detect,
 * such as `export { middleware }` or `export function middleware`, leaves the
 * module lazy and the middleware is dropped. Middleware in `routes/main.tsx`
